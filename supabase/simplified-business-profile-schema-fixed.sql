-- Simplified Business Profile Schema for ClientHandle (Fixed)
...
-- Add constraints for the simplified fields (without IF NOT EXISTS)
ALTER TABLE public.users ADD CONSTRAINT check_business_name_length CHECK (LENGTH(business_name) <= 100);
ALTER TABLE public.users ADD CONSTRAINT check_service_description_length CHECK (LENGTH(service_description) <= 150);
